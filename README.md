...
**Если порты 80/443 уже заняты другим сайтом** — SportChat добавится отдельным
виртуальным хостом через твой существующий nginx/apache (твой сайт не пострадает):

```bash
sudo bash proxy-nginx.sh спортчат.рф
```

Скрипт сам определит nginx или apache, создаст хост с WebSocket-поддержкой
(нужно для стриминга ответов) и выпустит сертификат через certbot.
